...
- `C:\kbo`처럼 Git 클론 내부에서 실행하면 BAT가 자동으로 루트 파이프라인을 사용해 `C:\kbo\data`를 갱신합니다.
- 압축본 단독 폴더에서 실행하면 bundle 내부 `data`를 갱신합니다.

```bash
npm run ml:helper-pc -- --from=20260331 --to=20260408 --baseUrl=https://kbo-predictor.vercel.app
```

`ml:helper-pc`는 실행 시 자동으로 다음 순서로 동작합니다.
1) 스냅샷 백필(`backfill-snapshots`)
2) ML 재학습(`retrain-daily`)
3) 세이버 튜닝(`tune-saber-weights`)

피처 변경으로 스냅샷을 완전 초기화하려면 `--resetSnapshots=true`를 추가하세요.

```bash
npm run ml:helper-pc -- --from=20260331 --baseUrl=https://kbo-predictor.vercel.app --resetSnapshots=true
```
